...
class ShortcutRepositoryImpl @Inject constructor(private val shortcutDao: ShortcutDao): ShortcutRepository {
    override fun getAllShortcuts(): Flow<List<Shortcut>> = shortcutDao.getAll().map { entities -> entities.map{ Shortcut(it.id, it.url, it.name) } }
